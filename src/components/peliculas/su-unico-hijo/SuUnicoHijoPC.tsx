  ...
  useEffect(() => {
    if (searchQuery.trim().length >= 2) {
      ...
      const themeMap: { [key: string]: string[] } = {
        ...
        biblia: ['biblia', 'milagros', 'abraham', 'isaac']
      };
      ...
      <nav className={`fixed top-0 w-full z-[130] transition-all duration-500 px-8 py-4 flex items-center justify-between ${isScrolled || searchQuery.length > 0 ? 'bg-black shadow-lg' : 'bg-gradient-to-b from-black via-black/60 to-transparent'}`}>
        ...
        <div className="flex items-center gap-6">
          <div className="flex gap-4 mr-4">
            <Link href={`/pelicula/su-unico-hijo`}><img src="https://static.wixstatic.com/media/859174_367960b11c1c44ba89cd1582fd1b5776~mv2.png" className="w-7 h-7 object-contain cursor-pointer hover:scale-110 transition-transform" alt="ES" /></Link>
            <Link href={`/en/movie/his-only-son`}><img src="https://static.wixstatic.com/media/859174_35112d9ffe234d6f9dcef16cf8f7544e~mv2.png" className="w-7 h-7 object-contain cursor-pointer hover:scale-110 transition-transform" alt="EN" /></Link>
            <Link href={`/pt/filme/seu-unico-filho`}><img src="https://static.wixstatic.com/media/859174_830f1c20656e4d44a819bedfc13a22cc~mv2.png" className="w-7 h-7 object-contain cursor-pointer hover:scale-110 transition-transform" alt="PT" /></Link>
          </div>
