...
export default function HeroBanner() {
  ...
  return (
    <section className="relative w-full h-[50vh] md:h-[95vh] bg-black overflow-hidden">
      <Slider ref={sliderRef} {...settings}>
        {banners.map((item) => (
          <div key={item.id} className="relative w-full h-[50vh] md:h-[95vh] outline-none">
            <Image src={item.bg} alt="Banner" fill className="object-cover md:object-center opacity-80" priority />
            
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent z-10" />
            <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-transparent to-transparent z-10" />
            
            <div className="absolute inset-0 z-20 flex flex-col justify-center px-4 md:px-20 mt-6 md:mt-12">
              {/* Logo reducido en móvil */}
              <div className="relative w-[140px] h-[50px] md:w-[380px] md:h-[140px] mb-2 md:mb-6">
                <Image src={item.logo} alt="Logo" fill className="object-contain object-left" />
              </div>
              
              {/* Descripción compacta */}
              <p className="max-w-[300px] md:max-w-[480px] text-white text-[11px] md:text-[17px] font-medium mb-4 md:mb-8 leading-tight md:leading-relaxed drop-shadow-md line-clamp-2 md:line-clamp-none">
                {item.desc}
              </p>
              
              {/* BOTONES EN UNA SOLA LÍNEA */}
              <div className="flex items-center gap-2 md:gap-4">
                <button style={{ backgroundColor: '#F09800' }} className="flex items-center justify-center gap-1 text-white px-3 md:px-7 py-1.5 md:py-2.5 rounded font-bold text-[10px] md:text-base shadow-xl">
                  <FaPlay className="text-[8px] md:text-xs" /> Ver
                </button>
                <button className="flex items-center justify-center gap-1 bg-white/10 text-white px-3 md:px-7 py-1.5 md:py-2.5 rounded font-bold backdrop-blur-md border border-white/20 text-[10px] md:text-base">
                  <HiOutlineInformationCircle className="text-sm md:text-xl" /> Info
                </button>
                <button style={{ backgroundColor: '#F09800' }} className="flex items-center justify-center gap-1 text-white px-3 md:px-7 py-1.5 md:py-2.5 rounded font-bold text-[10px] md:text-base shadow-lg">
      ...
      <style jsx global>{`
        .custom-dots { bottom: 20px !important; }
