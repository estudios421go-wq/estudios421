...
// 1. IMPORTANTE: Debemos incluir la constante 'banners' que faltaba
const banners = [
...
export default function HeroBanner() {
  ...
  return (
    <section className="relative w-full aspect-video bg-black overflow-hidden mb-6">
      <Slider ref={sliderRef} {...settings}>
        {/* 2. Al estar definida la constante arriba, 'item' ya no será 'any' y el error desaparecerá */}
        {banners.map((item) => (
          <div key={item.id} className="relative w-full aspect-video outline-none">
            <div className="relative w-full h-full">
               <Image src={item.bg} alt="Banner" fill className="object-contain opacity-90" priority />
            </div>
            
            <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent z-10" />
            
            <div className="absolute inset-0 z-20 flex flex-col justify-end px-4 pb-8">
              <div className="relative w-[100px] h-[32px] mb-2">
                <Image src={item.logo} alt="Logo" fill className="object-contain object-left" />
              </div>
              
              <p className="max-w-[180px] text-white text-[9px] font-medium mb-3 leading-tight drop-shadow-md">
                {item.desc}
              </p>
              
              <div className="flex items-center gap-2">
                <button style={{ backgroundColor: '#F09800' }} className="flex items-center justify-center gap-1 text-white px-3 py-1.5 rounded font-bold text-[9px]">
                  <FaPlay className="text-[7px]" /> Ver
                </button>
                <button className="flex items-center justify-center gap-1 bg-white/10 text-white px-3 py-1.5 rounded font-bold backdrop-blur-md border border-white/20 text-[9px]">
                  <HiOutlineInformationCircle className="text-xs" /> Info
                </button>
                <button style={{ backgroundColor: '#F09800' }} className="flex items-center justify-center gap-1 text-white px-3 py-1.5 rounded font-bold text-[9px]">
                  <BiDonateHeart className="text-xs" /> Donar
                </button>
              </div>
            </div>
          </div>
      ...
      <style jsx global>{`
        .custom-dots { bottom: 5px !important; z-index: 50 !important; }
        .custom-dots li button:before { color: white !important; font-size: 5px !important; }
