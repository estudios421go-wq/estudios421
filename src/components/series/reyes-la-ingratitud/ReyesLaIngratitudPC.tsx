...
const ReyesIngratitudPC = () => {
  ...
  useEffect(() => {
    ...
    const savedEp = localStorage.getItem('reyes_ingratitud_last_ep');
    if (savedEp) {
      const idx = parseInt(savedEp);
      if (idx < reyesIngratitudEpisodes.length) setCurrentIdx(idx);
    }
  ...
  useEffect(() => {
    if (searchQuery.trim().length >= 2) {
      ...
      const themeMap: { [key: string]: string[] } = {
        ...
        reyes: ['reyes', 'david', 'saul', 'salomon', 'decepcion', 'ingratitud', 'rechazo'],
  ...
  const openEpisode = (idx: number) => {
    ...
    localStorage.setItem('reyes_ingratitud_last_ep', idx.toString());
  };
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans select-none overflow-x-hidden text-left unselectable">
      ...
      <nav className={`fixed top-0 w-full z-[130] transition-all duration-500 px-8 py-4 flex items-center justify-between ${isScrolled || searchQuery.length > 0 ? 'bg-black shadow-lg' : 'bg-gradient-to-b from-black via-black/60 to-transparent'}`}>
        ...
        <div className="flex items-center gap-6">
          <div className="flex gap-4 mr-4">
            {[{ n: '', img: '367960b11c1c44ba89cd1582fd1b5776', p: '/serie/reyes-la-ingratitud' }, 
              { n: 'en', img: '35112d9ffe234d6f9dcef16cf8f7544e', p: '/en/serie/reyes-la-ingratitud' }, 
              { n: 'pt', img: '830f1c20656e4d44a819bedfc13a22cc', p: '/pt/serie/reyes-la-ingratitud' }].map((l) => (
      ...
      <div className="relative w-full h-[88vh]">
        <img src="https://static.wixstatic.com/media/859174_af83f39762954a149f18f003c00b74f7~mv2.jpg" className="w-full h-full object-cover" alt="Banner Reyes: La Ingratitud" />
        ...
        <div className="fixed inset-0 z-[1000] bg-[#050608] flex flex-col animate-fade-in">
          <div className="h-[12vh] min-h-[85px] px-12 flex items-center justify-between bg-gradient-to-b from-[#0a0b0d] to-[#050608] border-b border-white/5">
            <div className="flex flex-col border-l-4 border-[#FF8A00] pl-6 py-1 text-left">
              <span className="text-[10px] font-black text-[#FF8A00]/80 uppercase tracking-[0.5em] mb-1">Serie: Reyes: La Ingratitud</span>
          ...
          <div className="h-[13vh] min-h-[100px] px-16 bg-gradient-to-t from-[#0a0b0d] border-t border-white/5 flex items-center justify-between">
            ...
            <button disabled={currentIdx === reyesIngratitudEpisodes.length - 1} onClick={() => openEpisode(currentIdx + 1)} className="group flex items-center gap-6 disabled:opacity-5 transition-all">
...
export default ReyesIngratitudPC;
