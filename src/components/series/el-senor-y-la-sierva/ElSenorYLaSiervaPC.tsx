...
const siervaEpisodes = [
  { id: 1, title: "¿Quién es esa muchacha?", dur: "01:02:00", desc: "Llevada por Valéria, Elisa llega a su nueva casa. Después de una noche llena de emociones, Caius se enfurece con un flagrante.", thumb: "https://static.wixstatic.com/media/859174_dbb10d52f53a41f7aed2e39d783740fd~mv2.jpg", url: "https://ok.ru/videoembed/9975122102967" },
  { id: 2, title: "¿Son ustedes cristianos?", dur: "00:44:25", desc: "Con cariño, Elisa ayuda a Aelia, Vita y Zita mediante un juego. Dejando a su hermana intrigada, Caius es confrontado sobre un nuevo interés notado por Valéria.", thumb: "https://static.wixstatic.com/media/859174_78639d3a543b446a9096bf8f9165323f~mv2.jpg", url: "https://ok.ru/videoembed/9975245638327" },
  { id: 3, title: "No soy como las demás", dur: "00:55:05", desc: "En conversación con Cayo, Elisa se atreve a hacer una petición. Entre lágrimas, Mesalina se une a Quinto con un objetivo común.", thumb: "https://static.wixstatic.com/media/859174_0f9c8f80ffe749a7a0f935b6f2987cc2~mv2.jpg", url: "https://ok.ru/videoembed/9975489170103" },
  { id: 4, title: "Eres demasiado pura", dur: "00:57:56", desc: "Después de una confesión inesperada, Priscus le hace una petición a Máxima. Sola en casa, Elisa recibe una visita.", thumb: "https://static.wixstatic.com/media/859174_864da2c039c3414c905c12b6a560f692~mv2.jpg", url: "https://ok.ru/videoembed/10012444396215" },
  { id: 5, title: "Soy yo la que no sirve para ti", dur: "00:52:16", desc: "Con una información crucial en su poder, Mesalina amenaza a Caius. Entre lágrimas, Aelia hace una revelación a Elisa.", thumb: "https://static.wixstatic.com/media/859174_1f0a97203a7b45e5a7c2dbdd2530cdfa~mv2.jpg", url: "https://ok.ru/videoembed/10012444658359" },
  { id: 6, title: "¡Dios lo sabrá!", dur: "00:51:33", desc: "Entre sus sentimientos y su fe, Elisa es tentada por una petición de Caius. Sin poder ocultarlo más, Magnus hace una confesión a Valéria.", thumb: "https://static.wixstatic.com/media/859174_eaa98bbbac0544729f730688c65fc11c~mv2.jpg", url: "https://ok.ru/videoembed/10012444854967" },
  { id: 7, title: "Último día de la inauguración", dur: "00:44:41", desc: "Atendiendo una súplica arriesgada, Prisco va al encuentro de un prisionero. A petición insistente de Mesalina, Cayo permanece en el anfiteatro esperando una sorpresa.", thumb: "https://static.wixstatic.com/media/859174_2b24f30c2ed84b5595671880a6ee7868~mv2.jpg", url: "https://ok.ru/videoembed/10035173132983" },
  { id: 8, title: "Muerte a los cristianos", dur: "00:41:34", desc: "Con el pueblo de Roma expectante en el Coliseo, la euforia se ve interrumpida por una confesión inesperada. Discutiendo con Agripa, Isabella se sorprende con una revelación de su padre.", thumb: "https://static.wixstatic.com/media/859174_7aa1115accb944d5b4eb30688844be56~mv2.jpg", url: "https://ok.ru/videoembed/10035243649719" },
  { id: 9, title: "Pobre Caius", dur: "00:51:18", desc: "Hablando sobre el pasado, Magnus ayuda a Caius a tomar una decisión importante. Después de exponer su conflicto a Dios, Elisa recibe un consejo especial.", thumb: "https://static.wixstatic.com/media/859174_af006b8702d94e849f663c4e17d38201~mv2.jpg", url: "https://ok.ru/videoembed/10042889800375" },
  { id: 10, title: "Carta de Juan", dur: "00:51:46", desc: "Después del reencuentro con la familia de Caius, Elisa se sorprende con la llegada de otra mujer. Bajo la mirada del pueblo de Roma, Domiciano pone a un apóstol ante una decisión de vida o muerte.", thumb: "https://static.wixstatic.com/media/859174_4a19e8ad67544e4c8318b0450d370675~mv2.jpg", url: "https://ok.ru/videoembed/10042889996983" }
];

const ElSenorYLaSierva_PC = () => {
  ...
  useEffect(() => {
    ...
    const savedEp = localStorage.getItem('sierva_last_ep');
    if (savedEp) {
      const idx = parseInt(savedEp);
      if (idx < siervaEpisodes.length) setCurrentIdx(idx);
    }
  ...
  // --- LÓGICA DE BÚSQUEDA IDÉNTICA A LA SERIE MAESTRA ---
  useEffect(() => {
    if (searchQuery.trim().length >= 2) {
      ...
      const themeMap: { [key: string]: string[] } = {
        ...
        sierva: ['sierva', 'el señor y la sierva', 'roma', 'caius', 'elisa', 'cristianos'],
        biblia: ['biblia', 'milagros', 'apostol']
      };
  ...
  const openEpisode = (idx: number) => {
    ...
    localStorage.setItem('sierva_last_ep', idx.toString());
  };
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans select-none overflow-x-hidden text-left unselectable">
      ...
      <nav className={`fixed top-0 w-full z-[130] transition-all duration-500 px-8 py-4 flex items-center justify-between ${isScrolled || searchQuery.length > 0 ? 'bg-black shadow-lg' : 'bg-gradient-to-b from-black via-black/60 to-transparent'}`}>
        ...
        <div className="flex items-center gap-6">
          <div className="flex gap-4 mr-4">
            {[{ n: '', img: '367960b11c1c44ba89cd1582fd1b5776', p: '/serie/el-senor-y-la-sierva' }, 
              { n: 'en', img: '35112d9ffe234d6f9dcef16cf8f7544e', p: '/en/series/the-lord-and-the-handmaid' }, 
              { n: 'pt', img: '830f1c20656e4d44a819bedfc13a22cc', p: '/pt/serie/o-senhor-e-a-serva' }].map((l) => (
      ...
      <div className="relative w-full h-[88vh]">
        <img src="https://static.wixstatic.com/media/859174_b41c2cdbebd34030845e9904c1a979b7~mv2.jpg" className="w-full h-full object-cover" alt="Banner El Señor y la Sierva" />
        ...
        <div className="fixed inset-0 z-[1000] bg-[#050608] flex flex-col animate-fade-in">
          <div className="h-[12vh] min-h-[85px] px-12 flex items-center justify-between bg-gradient-to-b from-[#0a0b0d] to-[#050608] border-b border-white/5">
            <div className="flex flex-col border-l-4 border-[#FF8A00] pl-6 py-1 text-left">
              <span className="text-[10px] font-black text-[#FF8A00]/80 uppercase tracking-[0.5em] mb-1">Serie: El Señor y la Sierva</span>
              <h2 className="text-2xl font-black tracking-tighter uppercase leading-none">Capítulo {siervaEpisodes[currentIdx].id} <span className="text-white/10 mx-3">/</span> {siervaEpisodes[currentIdx].title}</h2>
            </div>
          ...
          <div className="h-[13vh] min-h-[100px] px-16 bg-gradient-to-t from-[#0a0b0d] border-t border-white/5 flex items-center justify-between">
            ...
            <button disabled={currentIdx === siervaEpisodes.length - 1} onClick={() => openEpisode(currentIdx + 1)} className="group flex items-center gap-6 disabled:opacity-5 transition-all">
...
export default ElSenorYLaSierva_PC;
