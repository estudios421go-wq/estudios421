...
const LeaMobile = () => {
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#F09800] overflow-x-hidden">
      ...
      {/* RENDERIZADO CONDICIONAL DE ALTA SEGURIDAD */}
      {selectedVideo ? (
        /* VISTA DEL REPRODUCTOR: SEPARADA POR COMPLETO */
        <div className="fixed inset-0 z-[9999] bg-black flex flex-col w-full h-full overscroll-none">
          <div className="h-[60px] px-6 flex items-center justify-between border-b border-white/5 bg-black">
            <div className="flex flex-col max-w-[70%] border-l-2 border-[#F09800] pl-3 text-left">
              <span className="text-[8px] font-black text-[#F09800] uppercase tracking-widest leading-none mb-1">Estudios 421</span>
              <span className="text-xs font-bold uppercase truncate">Ep. {leaEpisodes[currentIdx].id} — {leaEpisodes[currentIdx].title}</span>
            </div>
            <button onClick={() => setSelectedVideo(null)} className="text-4xl p-2 leading-none">&times;</button>
          </div>
          ...
          <div className="h-[100px] px-8 bg-black border-t border-white/5 flex items-center justify-between pb-6">
        ...
        /* VISTA DE LA PÁGINA: DISEÑO ORIGINAL INTACTO */
        <>
