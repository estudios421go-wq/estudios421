...
const LeaMobile = () => {
  ...
  useEffect(() => {
    const handleScroll = () => { if (!selectedVideo) setIsScrolled(window.scrollY > 10); };
  ...
  const openEpisode = useCallback((idx: number) => { 
    ...
    window.scrollTo(0, 0);
    // Bloqueamos el scroll del body para evitar que el navegador móvil se confunda
  ...
  // RENDERIZADO CONDICIONAL BLINDADO
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#F09800] overflow-x-hidden">
      <Head>
        <title>{selectedVideo ? `Reproduciendo Ep. ${leaEpisodes[currentIdx].id}` : 'Lea — Estudios 421'}</title>
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0"/>
      </Head>

      {/* CAPA DE VIDEO: Si existe video, esta capa "devora" a la otra */}
      {selectedVideo ? (
        <div className="fixed inset-0 z-[5000] bg-black flex flex-col w-full h-full animate-none">
          <div className="h-[10vh] min-h-[60px] px-6 flex items-center justify-between border-b border-white/5 bg-black">
            <div className="flex flex-col max-w-[70%] border-l-2 border-[#F09800] pl-3">
              <span className="text-[8px] font-black text-[#F09800] uppercase tracking-widest leading-none mb-1">En Reproducción</span>
              <span className="text-xs font-bold uppercase truncate tracking-tighter">EP. {leaEpisodes[currentIdx].id} — {leaEpisodes[currentIdx].title}</span>
            </div>
            <button onClick={closePlayer} className="text-4xl p-2 leading-none">&times;</button>
          </div>
          ...
          <div className="h-[15vh] min-h-[100px] px-8 bg-black border-t border-white/5 flex items-center justify-between pb-8">
            <button disabled={currentIdx === 0} onClick={() => openEpisode(currentIdx - 1)} className="flex flex-col items-center gap-1 active:scale-90 transition-transform disabled:opacity-5">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">Anterior</span>
            </button>
            <button onClick={closePlayer} className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
            ...
            <button disabled={currentIdx === leaEpisodes.length - 1} onClick={() => openEpisode(currentIdx + 1)} className="flex flex-col items-center gap-1 active:scale-90 transition-transform disabled:opacity-5">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-[#F09800]">Siguiente</span>
            </button>
          </div>
        </div>
      ) : (
        <>
          {/* INTERFAZ NORMAL (Solo se ve si NO hay video seleccionado) */}
          ...
          <div className="px-4 mt-10 mb-20 text-left">
            <header className="flex items-center gap-2 mb-4 border-b border-white/10 pb-2">
              <h2 className="text-xs font-bold text-gray-500 tracking-widest uppercase">Episodios Disponibles</h2>
            </header>
