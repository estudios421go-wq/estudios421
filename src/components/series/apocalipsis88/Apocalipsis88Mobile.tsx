...
// --- CONFIGURACIÓN DE EPISODIOS (88 CAPÍTULOS) ---
const apocalipsis88Episodes = [
...
const Apocalipsis88Mobile = () => {
  ...
  useEffect(() => {
    ...
    // LLAVE DE MEMORIA ÚNICA PARA ESTA VERSIÓN MÓVIL
    const saved = localStorage.getItem('apocalipsis_88_mobile_last_ep');
  ...
  const openEpisode = (idx: number) => {
    setSelectedVideo(apocalipsis88Episodes[idx]);
    setCurrentIdx(idx);
    localStorage.setItem('apocalipsis_88_mobile_last_ep', idx.toString());
  };
  ...
  if (selectedVideo) {
    return (
      <div className="fixed inset-0 z-[2000] bg-[#050608] flex flex-col overflow-hidden text-left">
        <Head><title>Episodio {currentIdx + 1} — Apocalipsis (88)</title></Head>
        <div className="px-6 h-[85px] flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent border-b border-white/5 z-10">
          <div className="flex flex-col border-l-4 border-[#F09800] pl-4 py-1">
            <span className="text-[9px] font-black text-[#F09800] uppercase tracking-[0.3em] mb-1">Apocalipsis (Resumida)</span>
        ...
        <div className="flex-grow flex flex-col relative bg-black items-center justify-center">
          ...
          <div className="absolute inset-x-0 bottom-8 flex justify-around items-center px-6">
            ...
            <button disabled={currentIdx === apocalipsis88Episodes.length - 1} onClick={() => openEpisode(currentIdx + 1)} className="flex flex-col items-center gap-2 group disabled:opacity-5">
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#F09800] text-left">
      <Head><title>Apocalipsis (88) — Estudios 421</title></Head>
      ...
      <div className={`fixed inset-0 bg-black/98 z-[100] transition-transform duration-500 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full pt-24 px-8 gap-8 text-left">
          ...
          <div className="flex gap-6">
            {/* RUTAS DE IDIOMA ACTUALIZADAS A LA VERSIÓN 88 */}
            {[{l:'ESP', i:'367960b11c1c44ba89cd1582fd1b5776', p:'/serie/apocalipsis-88'}, {l:'ENG', i:'35112d9ffe234d6f9dcef16cf8f7544e', p:'/en/serie/apocalipsis-88'}, {l:'PT', i:'830f1c20656e4d44a819bedfc13a22cc', p:'/pt/serie/apocalipsis-88'}].map((lang) => (
      ...
      <div className="relative w-full pt-0 bg-black shadow-2xl">
        <div className="w-full aspect-[4/3] relative">
          <img src="https://static.wixstatic.com/media/859174_f73133cc2267477786433f55120ea643~mv2.jpg" className="w-full h-full object-contain" alt="Banner Apocalipsis" />
      ...
      <div className="px-5 mt-12 mb-20 text-left">
        <header className="flex items-center gap-3 mb-2 border-b border-white/5 pb-3">
          <div className="w-1 h-5 bg-[#F09800]"></div>
          <h2 className="text-[11px] font-black text-white/50 tracking-[0.3em] uppercase">Episodios (Resumida)</h2>
        </header>

        <p className="text-[#F09800] font-black text-[10px] tracking-[0.1em] mb-6 animate-pulse uppercase">
          VERSIÓN COMPACTA DE 88 EPISODIOS
        </p>

        <div className="grid grid-cols-5 gap-3">
          {apocalipsis88Episodes.map((_, i) => (
...
export default Apocalipsis88Mobile;
