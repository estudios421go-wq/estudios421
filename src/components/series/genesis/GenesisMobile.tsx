...
const GenesisMobile = () => {
  ...
  const toggleMyList = () => {
    ...
    if (inMyList) { list = list.filter((id: string) => id !== 'genesis'); setInMyList(false); } 
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#F09800] text-left">
        ...
        <div className="flex flex-col h-full pt-24 px-8 gap-8">
          ...
          <div className="flex gap-6">
            {[{l:'ESP', i:'367960b11c1c44ba89cd1582fd1b5776'}, {l:'ENG', i:'35112d9ffe234d6f9dcef16cf8f7544e'}, {l:'PT', i:'830f1c20656e4d44a819bedfc13a22cc'}].map((lang) => (
              <button key={lang.l} className="flex flex-col items-center gap-2"><img src={`https://static.wixstatic.com/media/859174_${lang.i}~mv2.png`} alt={lang.l} className="w-10 h-10 object-contain" /><span className="text-[10px] text-white font-bold">{lang.l}</span></button>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* REPRODUCTOR PREMIUM (GLASSMORPHISM) */}
      {selectedVideo && (
        <div className="fixed inset-0 z-[2000] bg-[#050608] flex flex-col overflow-hidden text-left">
          <div className="px-6 h-[85px] flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent border-b border-white/5 z-10">
            <div className="flex flex-col border-l-4 border-[#F09800] pl-4 py-1">
              <span className="text-[9px] font-black text-[#F09800] uppercase tracking-[0.3em] mb-1">Serie: Génesis</span>
          ...
          <div className="flex-grow flex flex-col relative bg-black items-center justify-center">
            ...
            <div className="absolute inset-x-0 bottom-8 flex justify-around items-center px-6">
              <button disabled={currentIdx === 0} onClick={() => openEpisode(currentIdx - 1)} className="flex flex-col items-center gap-2 disabled:opacity-5">
              ...
              <button disabled={currentIdx === genesisEpisodes.length - 1} onClick={() => openEpisode(currentIdx + 1)} className="flex flex-col items-center gap-2 disabled:opacity-5">
