...
import TVNavbar from '../../tv/Navbar'; 
...
const GenesisTV = () => {
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans overflow-x-hidden selection:bg-[#F09800]">
      <Head><title>Génesis — Estudios 421 TV</title></Head>
      <TVNavbar />

      <div className="relative w-full h-[90vh]">
        <img src="https://static.wixstatic.com/media/859174_264be00ba6d14e699767e79c49297e5c~mv2.jpg" className="w-full h-full object-cover" alt="Génesis TV" />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/40" />
        <div className="absolute bottom-20 left-20 flex gap-10 z-10">
          <button onClick={() => { setSelectedVideo(genesisEpisodes[0].url); setCurrentIdx(0); }} className="bg-white text-black font-black py-8 px-20 rounded-md text-3xl border-8 border-transparent focus:border-[#F09800] focus:scale-110 transition-all outline-none">▶ VER AHORA</button>
          <button className="bg-white/10 text-white font-bold py-8 px-16 rounded-md text-3xl border-8 border-transparent focus:border-[#F09800] focus:bg-white/20 transition-all outline-none">+ MI LISTA</button>
          <button onClick={() => setShowQR(true)} className="bg-white/10 text-white font-bold py-8 px-16 rounded-md text-3xl border-8 border-transparent focus:border-[#F09800] focus:bg-white/20 transition-all outline-none">❤ DONAR (QR)</button>
        </div>
      </div>

      <div className="px-20 mt-10 mb-40">
        <h2 className="text-4xl font-black mb-12 text-gray-500 uppercase tracking-tighter border-b-8 border-white/5 pb-4">Capítulos Completos</h2>
        <div className="grid grid-cols-6 gap-12">
          {genesisEpisodes.map((ep, index) => (
            <button key={ep.id} className="flex flex-col gap-4 text-left border-8 border-transparent focus:border-[#F09800] focus:scale-110 transition-all outline-none rounded-xl p-3 bg-transparent focus:bg-white/5" onClick={() => { setSelectedVideo(ep.url); setCurrentIdx(index); }}>
              <div className="relative aspect-video rounded-lg overflow-hidden shadow-2xl border-4 border-white/5">
                <img src={ep.thumb} className="w-full h-full object-cover" />
                <span className="absolute bottom-3 right-3 bg-black px-4 py-1 text-base font-black rounded uppercase">{ep.dur}</span>
              </div>
              <h3 className="font-black text-xl truncate uppercase">EP. {ep.id} {ep.title}</h3>
            </button>
          ))}
        </div>
      </div>
      ...
      {selectedVideo && (
        <div className="fixed inset-0 z-[2000] bg-black flex flex-col items-center justify-center">
          ...
          <div className="absolute bottom-12 flex gap-12 bg-black/60 p-6 rounded-full backdrop-blur-xl border border-white/10">
            <button disabled={currentIdx === 0} onClick={() => { setCurrentIdx(currentIdx - 1); setSelectedVideo(genesisEpisodes[currentIdx - 1].url); }} className="text-white text-2xl font-black py-6 px-16 rounded-full border-8 border-transparent focus:border-[#F09800] outline-none">ANTERIOR</button>
            <button onClick={() => setSelectedVideo(null)} className="text-white text-2xl font-black py-6 px-16 rounded-full border-8 border-transparent focus:border-white outline-none">CERRAR</button>
            <button disabled={currentIdx === genesisEpisodes.length - 1} onClick={() => { setCurrentIdx(currentIdx + 1); setSelectedVideo(genesisEpisodes[currentIdx + 1].url); }} className="text-white text-2xl font-black py-6 px-16 rounded-full border-8 border-transparent focus:border-[#F09800] outline-none">SIGUIENTE</button>
          </div>
        </div>
      )}

      {showQR && (
        <div className="fixed inset-0 z-[3000] bg-black/95 flex items-center justify-center" onClick={() => setShowQR(false)}>
          <div className="flex flex-col items-center text-center">
            <h2 className="text-6xl font-black mb-12">DONACIÓN POR PAYPAL</h2>
            <div className="bg-white p-16 rounded-3xl shadow-2xl mb-12">
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${encodeURIComponent('https://www.paypal.com/donate/?hosted_button_id=C2Y74BGQB4HKS')}`} alt="QR TV" className="w-[500px] h-[500px]" />
            </div>
            <button autoFocus onClick={() => setShowQR(false)} className="bg-[#F09800] text-black font-black py-10 px-28 rounded-full text-4xl border-8 border-transparent focus:border-white transition-all outline-none uppercase">Cerrar Código</button>
          </div>
        </div>
      )}
      <Footer />
