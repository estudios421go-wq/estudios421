...
const InicioPC = () => {
  ...
  // --- LISTAS MANUALES (CÓDIGO 2 - FIDELIDAD TOTAL) ---
  const dataEstrenos = [
  ...
  useEffect(() => {
    if (searchQuery.trim().length >= 2) {
      ...
      Object.entries(themeMap).forEach(([key, values]) => {
        if (term.includes(key)) values.forEach(v => relatedTerms.add(v));
      });
  ...
  useEffect(() => {
    ...
    // Recomendados 10 aleatorios
    const pool = [...dataBiblicas, ...allSeries.filter(s => s.category === 'Película')];
    setRecommended([...pool].sort(() => 0.5 - Math.random()).slice(0, 10));
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#FF8A00]">
        ...
        <div className="flex items-center gap-6">
          <div className="flex gap-3 mr-2">
            {[{ n: '', img: '367960b11c1c44ba89cd1582fd1b5776' }, { n: 'en', img: '35112d9ffe234d6f9dcef16cf8f7544e' }, { n: 'pt', img: '830f1c20656e4d44a819bedfc13a22cc' }].map((l) => (
              <Link key={l.n} href={l.n === '' ? '/' : `/${l.n}`}><img src={`https://static.wixstatic.com/media/859174_${l.img}~mv2.png`} className="w-6 h-6 object-contain hover:scale-110 transition-transform cursor-pointer" /></Link>
            ))}
          </div>
      ...
      <main className="relative">
            ...
            {bannersData.map((item) => (
              <div key={item.id} className="relative w-full h-[95vh] outline-none">
                ...
                <div className="absolute inset-0 z-20 flex flex-col justify-center px-20">
                  ...
                  <p className="max-w-[550px] text-white text-[19px] mb-8 leading-relaxed opacity-90">{item.desc}</p>
                  <div className="flex items-center gap-5">
                    <button onClick={() => router.push(item.path)} className="bg-[#F09800] text-white px-9 py-3 rounded-md font-bold flex items-center gap-2 hover:scale-105 transition-all"><FaPlay size={12} /> Ver Ahora</button>
                    <button onClick={() => openTrailer(item.trailer, item.path)} className="bg-white/10 px-9 py-3 rounded-md font-bold border border-white/20 flex items-center gap-2 hover:scale-105 transition-all"><HiOutlineInformationCircle size={22} /> Más Información</button>
                    <a href="#" className="bg-black/40 px-6 py-3 rounded-md font-bold border border-[#F09800]/50 flex items-center gap-2 hover:scale-105 transition-all"><BiDonateHeart size={20} className="text-[#F09800]" /> Donar</a>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* CARRUSELES (CÓDIGO 2 + POSICIÓN CÓDIGO 1) */}
      ...
      <style jsx global>{`
        ...
        .custom-dots li.slick-active button:before { color: #F09800 !important; font-size: 10px !important; }
