...
const InicioPC = () => {
  ...
  // --- LISTAS MANUALES DEL SEGUNDO CÓDIGO (POSTERS EXACTOS) ---
  const dataEstrenos = [
  ...
  // --- LÓGICA DE BÚSQUEDA DEL CÓDIGO 1 ---
  useEffect(() => {
  ...
  // --- BARRA DE NAVEGACIÓN (CÓDIGO 1 ORIGINAL) ---
  const Navbar = (
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans selection:bg-[#FF8A00]">
      ...
      <main className="relative">
        {/* BUSCADOR OVERLAY ORIGINAL (CÓDIGO 1) */}
        {searchQuery.length > 0 && (
          <div className="fixed inset-0 bg-black z-[120] pt-24 px-8 md:px-16 overflow-y-auto pb-20">
             <h2 className="text-white text-2xl font-bold mb-10 uppercase tracking-widest flex items-center gap-3"><span className="w-1.5 h-6 bg-[#FF8A00]" />Resultados para: "{searchQuery}"</h2>
        ...
        {/* CARRUSELES POSICIONADOS SEGÚN ORDEN DIRECTO */}
      ...
      {/* FOOTER ORIGINAL DEL PRIMER CÓDIGO */}
      ...
      <style jsx global>{`
        ...
        .movie-slider .slick-prev, .movie-slider .slick-next { height: 100%; width: 50px; z-index: 100; top: 50%; transform: translateY(-50%); transition: all 0.4s ease; opacity: 0; background: rgba(0,0,0,0.6); }
        ...
        .movie-slider .slick-prev { left: 0 !important; border-radius: 0 8px 8px 0; }
        .movie-slider .slick-next { right: 0 !important; border-radius: 8px 0 0 8px; }
      `}</style>
    </div>
  );
};
