...
const TVNavbar = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] bg-gradient-to-b from-black via-black/90 to-transparent px-16 py-10 flex items-center justify-between">
      ...
      <div className="flex items-center gap-12">
        ...
        <div className="flex gap-4">
          {navLinks.map((link) => (
            <button 
              key={link} 
              className="px-8 py-3 text-white text-2xl font-bold rounded-md border-4 border-transparent focus:border-[#F09800] focus:bg-white/10 outline-none transition-all"
            >
              {link}
            </button>
          ))}
      ...
      {/* SECCIÓN DERECHA: IDIOMAS + BUSCADO + PERFIL */}
      <div className="flex items-center gap-10">
        ...
        <div className="flex gap-4 p-2 bg-white/5 rounded-xl">
          {languages.map((lang) => (
            <button 
              key={lang.name} 
              className="p-1 rounded-full border-4 border-transparent focus:border-[#F09800] outline-none transition-all transform focus:scale-125"
            >
        ...
        {/* BUSCADOR: Diseñado como un botón de bloque */}
        <button 
          className="flex items-center gap-4 bg-white/10 px-8 py-3 rounded-full border-4 border-transparent focus:border-[#F09800] focus:bg-white/20 outline-none text-white transition-all shadow-lg"
        >
