...
const genesisEpisodes = [
  { id: 1, title: "El edén", dur: "43:16", desc: "Esta es la historia de como todo fue creado por Dios y el inicio de la humanidad con Adán y Eva, el primer hombre y mujer en la tierra.", thumb: "https://static.wixstatic.com/media/859174_34318cd5b639416e9f54aac740e60a84~mv2.jpg", url: "https://ok.ru/videoembed/13888818973184" },
  { id: 2, title: "Las consecuencias", dur: "43:09", desc: "Adán y Eva tienen que vivir las consecuencias de haber comido el fruto prohibido. Fueron expulsados del paraíso.", thumb: "https://static.wixstatic.com/media/859174_40d433337bf542d68b9965dccb4f0ecd~mv2.jpg", url: "https://ok.ru/videoembed/13888837454336" },
  { id: 3, title: "Hermanos", dur: "42:59", desc: "Caín y Abel son los hijos de Adán y Eva y ambos tratan de sobrevivir, pero son muy distintos.", thumb: "https://static.wixstatic.com/media/859174_244fa62379ba4682a6cc03815f3891f6~mv2.jpg", url: "https://ok.ru/videoembed/13888884902400" },
  { id: 4, title: "La opción", dur: "43:00", desc: "Caín y Abel logran llamar la atención de Dios, pero Caín está seguro que solo respondió a los regalos de Abel.", thumb: "https://static.wixstatic.com/media/859174_461f6b43b4a7488fb1eca919f954f145~mv2.jpg", url: "https://ok.ru/videoembed/13888885230080" },
...
export default function GenesisPage() {
  ...
  return (
    <div className="bg-black min-h-screen text-white font-sans overflow-x-hidden">
      ...
      <div className="relative w-full h-[80vh] md:h-[916px] flex items-end">
        <img 
        ...
        {/* BOTONES SOLAPADOS AL BORDE DEL BANNER */}
        <div className="relative z-10 px-8 pb-12 flex flex-wrap gap-4 -mb-8">
          <button className="bg-white text-black font-bold py-3 px-10 rounded-md flex items-center gap-2 hover:bg-gray-200 transition scale-110 shadow-2xl">
            ▶ VER AHORA
          </button>
          <button className="bg-gray-700/60 hover:bg-gray-600 text-white font-bold py-3 px-8 rounded-md backdrop-blur-md border border-white/10 transition shadow-2xl">
            + MI LISTA
          </button>
          <button 
            onClick={() => window.open('https://www.paypal.com/donate/?hosted_button_id=C2Y74BGQB4HKS', '_blank')}
            className="bg-gray-700/60 hover:bg-gray-600 text-white font-bold py-3 px-8 rounded-md backdrop-blur-md border border-white/10 transition shadow-2xl"
          >
            ❤ DONAR
          </button>
        </div>
      ...
      {/* LISTA DE EPISODIOS */}
      <div className="px-8 mt-24 mb-32">
        <h2 className="text-2xl font-semibold mb-8 text-gray-200 uppercase tracking-widest">Capítulos Completos</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-8">
          {genesisEpisodes.map((ep) => (
            ...
            >
              <div className="relative aspect-video rounded-lg overflow-hidden border border-white/5 group-hover:border-white/30 transition shadow-lg">
                <img src={ep.thumb} className="w-full h-full object-cover transform group-hover:scale-110 transition duration-500" />
                <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition" />
                <div className="absolute bottom-2 right-2 bg-black/80 px-2 py-0.5 text-[10px] font-bold rounded border border-white/10">{ep.dur}</div>
              </div>
              <div className="px-1">
                <h3 className="font-bold text-sm leading-tight group-hover:text-orange-400 transition">EP. {ep.id} {ep.title}</h3>
                <p className="text-[11px] text-gray-400 line-clamp-2 mt-1 leading-normal">{ep.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* MODO CINE / REPRODUCTOR */}
      {selectedVideo && (
        <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-xl flex flex-col animate-fade-in">
          <div className="p-4 flex justify-end">
            <button onClick={() => setSelectedVideo(null)} className="text-white hover:text-gray-400 transition">
              <span className="text-5xl font-light">&times;</span>
            </button>
          </div>
          <div className="flex-grow flex items-center justify-center p-2 md:p-10">
            <iframe 
              src={selectedVideo} 
              className="w-full h-full max-w-6xl aspect-video rounded-xl shadow-2xl border border-white/10" 
              allowFullScreen 
            />
          </div>
        </div>
